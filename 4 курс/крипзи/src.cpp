			...
			vector<string> rk)
{
	...
	for (int i = 0; i < 16; i++) {
		// Expansion D-box
		cout<<"Round: "<<i+1<<endl;
		cout<<"Left and Right: "<<bin2hex(left)<<":"<<bin2hex(right)<<endl;
		string right_expanded = permute(right, exp_d, 48);
		cout<<"Expansion: "<<bin2hex(right_expanded)<<endl;
		...
		cout<<"Before premute: "<<bin2hex(op)<<endl;
		// Straight D-box
		op = permute(op, per, 32);
		cout<<"After permute: "<<bin2hex(op)<<endl;
		// XOR left and op
		x = xor_(op, left);
		...
		cout << "Round End: " << i + 1 << " " << bin2hex(left)
			<< " " << bin2hex(right) << " " << rk[i]
			<< endl;
	}
...
int main(int argc, char const *argv[]) {
	...
	if(argc!=3) {
        printf("You're fucking idiot! 2 argument - MainKey and plaintext");
        return 1;
    }
	...
	string pt(argv[2]);
	// Hex to binary
	mainKey = hex2bin(mainKey);
	cout<<"Main Key 56 before permute: "<<mainKey<<endl;
	...
	string key_56 = permute(mainKey, keyp, 56); // key without parity
	...
	int shift_RIGHT_table[16] = { 0, 1, 2, 2, 2, 2, 2, 2,
							1, 2, 2, 2, 2, 2, 2, 1 };
	...
	string right = key_56.substr(28, 28);

	cout<<"left: "<< left<<" right: "<<right <<endl;
	cout<<"leftHex: "<< bin2hex(left) <<" rightHex: "<<bin2hex(right) <<endl;
	...
	for (int i = 0; i < 16; i++) {
		...
		cout<<"roundKeyHex: "<<bin2hex(RoundKey)<<endl;
